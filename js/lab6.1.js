    width = 500;
height = 300;

padding = 20;

dataSet = [14, 5, 26, 23, 9, 21, 7, 19, 22, 16, 2, 10];

// Scales
// xScale is a band scale that assigns an x-position to each data point, evenly spaced across the width of the chart
xScale = d3.scaleBand().domain(d3.range(dataSet.length)).rangeRound([padding, width - padding]).paddingInner(0.05);
// yScale is a linear scale that maps data values to y-positions, inverted to start from top
yScale = d3.scaleLinear().domain([d3.max(dataSet), 0]).rangeRound([padding, height]);

// Axes
// xAxis represents the x-axis of the chart, positioned at the bottom
xAxis = d3.axisBottom()
    .scale(xScale)
    .ticks(dataSet.length);

// yAxis represents the y-axis of the chart, positioned on the left
yAxis = d3.axisLeft()
    .scale(yScale)
    .ticks(10);

// SVG
// Create an SVG element to hold the bar chart
svg = d3.select("#container")
    .append("svg")
    .attr("width", width)
    .attr("height", height + padding);

// Bars
// Bind the data to rectangle elements (bars)
svg.selectAll("rect")
    .data(dataSet)
    .enter()
    .append("rect")
    .attr("x", function (d, i) {
        return xScale(i); // Position each bar based on its index
    })
    .attr("y", function (d) {
        return yScale(d); // Set y-position based on data value
    })
    .attr("width", xScale.bandwidth()) // Set width of each bar based on scale bandwidth
    .attr("height", function (d) {
        return height - yScale(d); // Calculate height from data value
    })
    .style("fill", "slategray") // Set initial color of bars
    .on("mouseover", function (event, d) {
        // Handle mouseover event to add a tooltip and change bar color
        let xPosition = parseFloat(d3.select(this).attr("x"));
        let yPosition = parseFloat(d3.select(this).attr("y"));

        // Add tooltip text to show data value
        svg.append("text")
            .attr("id", "tooltip")
            .attr("x", d > 9 ? xPosition + xScale.bandwidth() / 3 - 3 : xPosition + xScale.bandwidth() / 3)
            .attr("y", yPosition + 15)
            .text(d);

        // Transition to change bar color on hover
        d3.select(this)
            .transition()
            .duration(150)
            .style("fill", "orange");
    })
    .on("mouseout", function (d) {
        // Handle mouseout event to remove tooltip and reset bar color
        d3.select("#tooltip").remove();

        d3.select(this)
            .transition()
            .duration(150)
            .style("fill", "slategray");
    });

// Axes
// Append x-axis to the SVG
svg.append("g")
    .attr("class", "x-axis")
    .attr("transform", "translate(0," + (height) + ")") // Position x-axis at the bottom
    .call(xAxis);

// Append y-axis to the SVG
svg.append("g")
    .attr("transform", "translate(" + padding + ",0)") // Position y-axis with padding from the left
    .call(yAxis);

// Buttons for adding data
// Select the button and set up the click event to add new data
// .btn-l-5-1 is a class for the button that adds data to the chart
d3.select(".btn-l-5-1")
    .on("click", function () {
        let maxValue = 25;

        // Generate a random number to add to the data set
        let newNumber = Math.floor(Math.random() * maxValue) + 1;
        dataSet.push(newNumber);

        // Update xScale domain to include new data length
        xScale.domain(d3.range(dataSet.length));
        let bars = svg.selectAll("rect")
            .data(dataSet);

        // Enter new bars for the added data
        bars.enter()
            .append("rect")
            .attr("x", function (d, i) {
                return width; // Start new bars at the right edge
            })
            .attr("y", function (d) {
                return yScale(d); // Set y-position based on data value
            })
            .attr("height", function (d) {
                return height - yScale(d); // Set bar height based on data value
            })
            .style("fill", "slategray")
            .on("mouseover", function (event, d) {
                // Handle mouseover event for new bars
                let xPosition = parseFloat(d3.select(this).attr("x"));
                let yPosition = parseFloat(d3.select(this).attr("y"));

                // Add tooltip text to show data value
                svg.append("text")
                    .attr("id", "tooltip")
                    .attr("x", d > 9 ? xPosition + xScale.bandwidth() / 3 - 3 : xPosition + xScale.bandwidth() / 3)
                    .attr("y", yPosition + 15)
                    .text(d);

                // Change bar color on hover
                d3.select(this)
                    .transition()
                    .duration(150)
                    .style("fill", "orange");
            })
            .on("mouseout", function (d) {
                // Handle mouseout event for new bars
                d3.select("#tooltip").remove();

                d3.select(this)
                    .transition()
                    .duration(150)
                    .style("fill", "slategray");
            })
            .merge(bars) // Merge new bars with existing bars
            .transition() // Apply transition to adjust positions
            .duration(500)
            .attr("x", function (d, i) {
                return xScale(i); // Set new x-position for all bars
            })
            .attr("width", xScale.bandwidth());

        // Update x-axis to reflect changes in the data set
        svg.select(".x-axis")
            .transition()
            .duration(500)
            .call(xAxis);
    });

// Buttons for removing data
// Select the button and set up the click event to remove data
// .btn-l-5-2 is a class for the button that removes data from the chart
d3.select(".btn-l-5-2")
    .on("click", function () {
        // Remove the last data point from the data set
        dataSet.pop();

        // Update xScale domain to include updated data length
        xScale.domain(d3.range(dataSet.length));

        let bars = svg.selectAll("rect")
            .data(dataSet);

        // Remove the bar corresponding to the removed data point
        bars.exit()
            .filter(function(d, i) { return i === dataSet.length; })
            .transition()
            .duration(500)
            .attr("x", width) // Move bar out of view
            .remove();

        // Update existing bars
        bars.enter()
            .append("rect")
            .attr("x", function (d, i) {
                return width; // Start new bars at the right edge
            })
            .attr("y", function (d) {
                return height - yScale(d); // Set y-position for new bars
            })
            .merge(bars) // Merge new bars with existing bars
            .transition() // Apply transition to adjust positions and heights
            .duration(500)
            .attr("x", function (d, i) {
                return xScale(i); // Set new x-position for all bars
            })
            .attr("y", function (d) {
                return yScale(d); // Set y-position based on data value
            })
            .attr("width", xScale.bandwidth()) // Set bar width based on scale bandwidth
            .attr("height", function (d) {
                return height - yScale(d); // Set bar height based on data value
            });

        // Update x-axis to reflect changes in the data set
        svg.select(".x-axis")
            .transition()
            .duration(500)
            .call(xAxis);
    });